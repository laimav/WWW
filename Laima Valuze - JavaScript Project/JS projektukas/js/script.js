                        ...
                        <div class="row">
                            <div class="nine columns">
                                ...
                                <div class="three columns">
                                    <img src="img/${Receptas.nuotrauka}" alt="${Receptas.pavadinimas}">
                                </div>
                                <p>PORCIJŲ SKAIČIUS: <select id="mySelect" onchange="calculate(${Receptas.id})">
                                                    ${porcijuSk}                                           </select></p>
                                <p>PARUOŠIMO LAIKAS: ${Receptas.trukme}</p>
                                <p>INGREDIENTAI:</p>
                            </div>
                        </div>
                    </div>`;
...
function randomRecipe() {
    var randomRecipe = recipesArr[Math.floor(Math.random() * recipesArr.length)].arrTitle;
    document.getElementById("dienosReceptas").innerHTML =
        `<h1 onclick="printRecipe(${randomRecipe})">Dienos receptas!</h1>`;
    // printRecipe(recipesArr[Math.floor(Math.random()*recipesArr.length)].arrTitle); 
}

// ON MOUSE OVER 
function onMouseOver() {
    document.getElementById("dienosReceptas").setAttribute("title", "Spustelėkite du kartus");
}


function OnContextMenu() {
    alert("Jeigu norite atidaryti dienos receptą, du kartus spustelėkite pagrindiniame puslapyje esantį tekstą");
}
